...
class RestoItem extends HTMLElement {
  ...
  render() {
    this.innerHTML = `
      <div class="card">
        <div class="card-img">
          <img class="lazyload skeleton" data-src="${CONFIG.BASE_IMAGE_URL}${this._restoItem.pictureId}" alt="Foto ${this._restoItem.name}">            
        </div>
        <div class="card-body">
          <h2 class="card-title">
            <a class="card-title-item"
              href="${`/#/detail/${this._restoItem.id}`}" 
              aria-label="Name place, ${this._restoItem.name}" >
              ${this._restoItem.name}
            </a>
          </h2>
